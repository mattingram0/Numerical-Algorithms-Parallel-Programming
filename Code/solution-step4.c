 ...
 */
void updateBody() {
	const double EPSILON_4 = 1.65 * pow(10, -21) * -4.0;
	const double SIGMA = 3.4 * pow(10, -10);

	//Define our minimum time step. From the convergence plot, it appears that 1e-4 is a suitable small time step to ensure stability
	const double MIN_TIME_STEP = 1e-4;
	...
	for(i = 0; i < NumberOfBodies; i++){ 
		for(j = i + 1; j < NumberOfBodies; j++){
			...
			lj = EPSILON_4 * ((-12.0 * sr12) + (6.0 * sr6)) / r;
	...
	} else if(timeStepSize * maxV > minDx && timeStepSize / 8.0 > 10e-18){
		//Lazy evaluation should help us here - the right hand condition should only ever be calculated if the LHS succeeds	
		while (timeStepSize * maxV > minDx && timeStepSize / 8.0 > 10e-18){
			timeStepSize /= 8.0;
		}
	} else {
		timeStepSize *= 1.01;
